...
st.markdown("""
Ce calculateur vous permet d'estimer l’énergie thermique libérée en cas d'incendie pour différents éléments installés dans un tunnel (câbles, cloisons, revêtements, etc.), ainsi que de générer une courbe HRR (Heat Release Rate) et d’évaluer la contribution au feu selon la distance d’exposition. Vous pouvez également analyser le risque d'inflammation en fonction du flux thermique reçu, selon un seuil critique propre au matériau.
""")
...
if nom_projet:
    ...
    # --- Suite du code à reprendre ici pour l'encodage matériaux, distance, alpha, résultats ---
else:
    st.warning("Veuillez d'abord renseigner un nom de projet pour activer l'application.")
